...
const material = new THREE.ShaderMaterial({
  extensions: {
    derivatives: true || '#extension GL_EOS_standart_derivatives : enable'
  },
  side: THREE.DoubleSide,
  uniforms: {
    time: { value: 0 },
    noiseScale: { value: 4 },
    speedAnimation: { value: 10000 },
    sky: { value: new THREE.TextureLoader().load('./abstract1.png') }
  },
  vertexShader: `
    uniform float time;
    varying vec2 vUv;
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec2 fakeUV = vec2(dot(vec3(1.0), vNormal), dot(vec3(-1., 0., 1.), vNormal));
      
      vec4 txt = texture2D(sky, newFakeUV + 0.9 * cnoise(vec4(vUv * noiseScale, time/speedAnimation, 0.0), vec4(10.)));
...
@Component
export default class WavesTheme extends Vue {
  stopAnimation = false
  ...
  init() {
    const canvas = this.$refs.main as HTMLCanvasElement
    ...
    const planeGeometry = new THREE.PlaneGeometry(planeWidthAtDistance, planeHeightAtDistance, 1, 1)
  ...
  updateSize() {
    ...
    plane.geometry = new THREE.PlaneGeometry(planeWidthAtDistance, planeHeightAtDistance, 1, 1)
    plane.position.z = planeZ
